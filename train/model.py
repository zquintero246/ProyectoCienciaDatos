...
class BiLSTMClassifier(nn.Module):
    def __init__(self, input_dim=225, hidden_dim=256, num_layers=2, num_classes=32, dropout=0.3, bidirectional=True):
        ...
        self.pool = nn.AdaptiveAvgPool1d(1)  # will use on features after permute
    ...
    def forward(self, x, lengths=None):
        ...
        if lengths is not None:
            ...
            out, _ = self.lstm(x)
        # out: (B, T, hidden*dirs)
        # do average pooling over time (masking done outside)
        # compute mean over valid timesteps if lengths provided
        if lengths is not None:
            # masked mean
